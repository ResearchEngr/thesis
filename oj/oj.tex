...
\draw (0,3.5) node(demand)[rectangle,draw]{ $\bD$};
\draw (6,3.5) node(contingency)[rectangle,draw]{ $\Xi$};
\draw (8,3.5) node(evolution)[rectangle,draw]{ $\Omega$};

\draw (0,2) node(rd){ $\rd $};
\draw (2,2) node(rf){ $\ry$};
\draw (4,2) node(grf){ $g(\ry)$};
\draw (6,2) node(rxi){ $\rxi$};
\draw (8,2) node(romega){ $\romega$};
\draw (4,.5) node(rlambda){ $\lambda(\rd,\rxi,\romega)$};
...
\draw[thick,->] (rd) -- (rlambda);
\draw[thick,->] (rxi) -- (rlambda);
\draw[thick,->] (romega) -- (rlambda);
...
The JCC model incorporates uncertainty from random demand, $\bD$, and initial contingencies $\Xi$.  The OPA model can include uncertainty from all three sources, $\bD, \Xi, \Omega$.  Let $\cX = \bD \times \Xi \times \Omega$ represent the whole probability space for the OPA model, where $\rchi = (\ri, \rxi, \romega)$ is an event in the whole sample space.  The random variables dependcies are laid out in figure (ref) that describe the relationships between variables.
...
\EndFor
\EndWhile
\State \label{done}Load Shed $  \lambda\left(x,\delta^m,xi,omega\right)  = z_{s^*} - z_0$
\EndProcedure
\end{algorithmic}
\end{algorithm}
...
\begin{alignat}{3}
\textbf{JCC OW:= }\min_{\left(x,\beta,;\theta,y,\pi,s,z\right)} & \displaystyle\sum_j \left[  c_2 \left(x_j^2 + \beta_j^2 \sD \right) + c_1 x_j + c_0 \right] &&  \label{ow_obj}  \\
&\cref{jcc_cons,jcc_kcl,jcc_limit,jcc_gen1,jcc_gen2,jcc_slack,jcc_pi,jcc_var,jcc_lr,jcc_risk}   && \nonumber\\
&\cref{n1meanup,n1meandown,n1stddev,n1linerisk}   && \nonumber\\
&\sum_e \eta_e z_{en} \leq \zeta_n && \forall n
\end{alignat}
\end{subequations}
